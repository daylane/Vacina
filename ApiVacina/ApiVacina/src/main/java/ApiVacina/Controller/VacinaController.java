...
public class VacinaController {
    ...
    @GetMapping
    public ResponseEntity<List<Vacina>> listarVacinas(@RequestParam(required = false) String fabricante, String vacina) {
        try {
            logger.info("Recebida uma solicitação para listar vacinas");
            return ResponseEntity.ok().body(vacinaService.listarVacinas(fabricante,vacina));
    ...
    @PostMapping("/vacinasmock")
    public ResponseEntity<Void> vacinasMock(){
        vacinaService.mockVacinas();
        return ResponseEntity.created(null).build();
    }


    @PutMapping("/{id}")
    public ResponseEntity<Vacina> atualizarVacina(@RequestBody @Valid Vacina VacinaDto, @PathVariable String id) {
        try {
    ...
    public ResponseEntity<String> deletarVacina(@PathVariable String id) {
        ...
        try {
            logger.info("Recebendo solicitação para excluir vacina com ID: {}", id);
            ...
            logger.error("Erro ao processar a solicitação de exclusão da vacina.");
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Erro ao excluir a vacina", ex);
        }
    }
}
